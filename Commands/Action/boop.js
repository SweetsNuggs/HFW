const {boopP} = require('../../Tools/jsons/actions.json')

module.exports = {
    name: 'boop',
    usage: ['Give someone a boopin', '```{prefix}boop <member mention>```'],
    enabled: true,
    aliases: [],
    category: 'Action',
    memberPermissions: [],
    ...
    async execute(client, message, args, data){
        ...
        const boop = boopP[Math.round(Math.random() * boopP.length)]
        
        const embed = {
            title: `${member.displayName} You were booped!`,
            description: `\`${message.member.displayName} gave ${member.displayName} a heckin' boop!\``,
            url: boop,
            image: {
                url: boop
            }

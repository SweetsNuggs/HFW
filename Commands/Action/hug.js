const {hugP} = require('../../Tools/jsons/actions.json')

module.exports = {
    name: 'hug', 
    usage: ['Give someone a big ol` hug!', '```{prefix}hug <member mention>```'],
    enabled: true,
    aliases: [],
    category: 'Action', 
    memberPermissions: [],
    ...
    async execute(client, message, args, data){
        ...
        const hug = hugP[Math.round(Math.random() * hugP.length)]


        const embed = {
            title: `${member.displayName} and ${message.member.displayName} are hugging it out!`,
            description: `\`How Sweet is it that ${message.member.displayName} and ${member.displayName} are hugging?!\``,
            url: hug, 
            image: {
                url: hug
            }
